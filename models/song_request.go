...
var SongRequestTableColumns = struct {
	ID                string
	SongID            string
	SongName          string
	RequestedByUserID string
	...
	ID:                "song_request.id",
	SongID:            "song_request.song_id",
	SongName:          "song_request.song_name",
	RequestedByUserID: "song_request.requested_by_user_id",
	UsernameAtTime:    "song_request.username_at_time",
	GuildID:           "song_request.guild_id",
	GuildNameAtTime:   "song_request.guild_name_at_time",
	RequestedAt:       "song_request.requested_at",
	PlayedAt:          "song_request.played_at",
	Played:            "song_request.played",
}
...
func (r *songRequestR) GetGuild() *DiscordServer {
	if r == nil {
		return nil
	}
	return r.Guild
...
func (r *songRequestR) GetSong() *Song {
	if r == nil {
		return nil
	}
	return r.Song
...
var (
	...
	songRequestColumnsWithoutDefault = []string{"song_name", "requested_by_user_id", "username_at_time", "guild_id", "guild_name_at_time"}
	songRequestColumnsWithDefault    = []string{"id", "song_id", "requested_at", "played_at", "played"}
	songRequestPrimaryKeyColumns     = []string{"id"}
	songRequestGeneratedColumns      = []string{}
)

type (
	// SongRequestSlice is an alias for a slice of pointers to SongRequest.
	// This should almost always be used instead of []SongRequest.
...
func (songRequestL) LoadGuild(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSongRequest interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeSongRequest.(*SongRequest)
		if !ok {
			object = new(SongRequest)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeSongRequest)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeSongRequest))
			}
		}
	} else {
		s, ok := maybeSongRequest.(*[]*SongRequest)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeSongRequest)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeSongRequest))
			}
		}
	}
...
func SongRequests(mods ...qm.QueryMod) songRequestQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"song_request\".*"})
	}

	return songRequestQuery{q}
}
...
func FindSongRequest(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*SongRequest, error) {
	...
	if err = songRequestObj.doAfterSelectHooks(ctx, exec); err != nil {
		return songRequestObj, err
	}
...
// Exists checks if the SongRequest row exists.
func (o *SongRequest) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return SongRequestExists(ctx, exec, o.ID)
}
