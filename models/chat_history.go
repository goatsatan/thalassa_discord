...
var ChatHistoryTableColumns = struct {
	...
	ID:                     "chat_history.id",
	UserID:                 "chat_history.user_id",
	UsernameAtTime:         "chat_history.username_at_time",
	GuildID:                "chat_history.guild_id",
	GuildNameAtTime:        "chat_history.guild_name_at_time",
	GuildChannelID:         "chat_history.guild_channel_id",
	GuildChannelNameAtTime: "chat_history.guild_channel_name_at_time",
	MessageID:              "chat_history.message_id",
}
...
func (r *chatHistoryR) GetGuild() *DiscordServer {
	if r == nil {
		return nil
	}
	return r.Guild
...
func (o *ChatHistory) Guild(mods ...qm.QueryMod) discordServerQuery {
	...
	return DiscordServers(queryMods...)
}
...
func (chatHistoryL) LoadGuild(ctx context.Context, e boil.ContextExecutor, singular bool, maybeChatHistory interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeChatHistory.(*ChatHistory)
		if !ok {
			object = new(ChatHistory)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeChatHistory)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeChatHistory))
			}
		}
	} else {
		s, ok := maybeChatHistory.(*[]*ChatHistory)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeChatHistory)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeChatHistory))
			}
		}
	}
...
func ChatHistories(mods ...qm.QueryMod) chatHistoryQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"chat_history\".*"})
	}

	return chatHistoryQuery{q}
}
...
func FindChatHistory(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*ChatHistory, error) {
	...
	if err = chatHistoryObj.doAfterSelectHooks(ctx, exec); err != nil {
...
// Exists checks if the ChatHistory row exists.
func (o *ChatHistory) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return ChatHistoryExists(ctx, exec, o.ID)
}
