...
func testSongToManySongRequests(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.SongRequests().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := SongSlice{&a}
	if err = a.L.LoadSongRequests(ctx, tx, false, (*[]*Song)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.SongRequests); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.SongRequests = nil
	if err = a.L.LoadSongRequests(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.SongRequests); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testSongToManyAddOpSongRequests(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddSongRequests(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.SongRequests[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.SongRequests[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.SongRequests().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testSongToManySetOpSongRequests(t *testing.T) {
	var err error
	...
	err = a.SetSongRequests(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetSongRequests(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.SongRequests[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.SongRequests[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testSongToManyRemoveOpSongRequests(t *testing.T) {
	var err error
	...
	err = a.AddSongRequests(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveSongRequests(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.SongRequests) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.SongRequests[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.SongRequests[0] != &e {
...
var (
	songDBTypes = map[string]string{`ID`: `text`, `Platform`: `text`, `SongName`: `text`, `Description`: `text`, `URL`: `text`, `DurationInSeconds`: `integer`, `IsStream`: `boolean`, `ThumbnailURL`: `text`, `Artist`: `text`, `Album`: `text`, `Track`: `text`}
	_           = bytes.MinRead
)
